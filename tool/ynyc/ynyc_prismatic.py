...
tb = 1 # flow duration in hours for HEC-RAS
ts = 5 # flow time step (minutes) for HEC-RAS
...
dp = 6 # significant decimal positions for report
...
results = funcs.results(dict['app_version'], datetime.now(), q, g, b, z1, z2, so, n, alpha, rho, y1aux, y2aux, steps, y2b, y2, funcs.shape_type(b, z1, z2), unit_sys, dicts, units, z, l, rcx, rcy, tb, ts, tpp, cell_size, dp)
print(results)
